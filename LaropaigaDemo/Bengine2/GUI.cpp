...
namespace Bengine2 {
	...
	void GUI::init(const std::string& resourceDirectory) {
		...
		if (m_renderer == nullptr) {
			...
			CEGUI::DefaultResourceProvider* rp = static_cast<CEGUI::DefaultResourceProvider*>(CEGUI::System::getSingleton().getResourceProvider());	//gets a global system object, singleton pattern	//original typecast: (CEGUI::DefaultResourceProvider*), not safe, switched to static cast
		...
		switch (evnt.type) {
			...
			case SDL_TEXTINPUT:
				...
				for (int i = 0; evnt.text.text[i] != '\0'; i++) {
					codePoint |= (((CEGUI::utf32)evnt.text.text[i]) << (i * 8)); //if we have one iteration it keeps it in the far right byte, second iteration stores it in the second to right byte, etc, bitiwse or (|=)
				}
	...
	CEGUI::Window* GUI::createWidget(const std::string& type, const glm::vec4& destRectPerc, const glm::vec4& destRectPixels, const std::string& name/* = ""*/) {
		CEGUI::Window* newWindow = CEGUI::WindowManager::getSingleton().createWindow(type, name);
		m_root->addChild(newWindow);
		setWidgetDestRect(newWindow, destRectPerc, destRectPixels);
		return newWindow;
	}
	...
	void GUI::setWidgetDestRect(CEGUI::Window* widget, const glm::vec4& destRectPerc, const glm::vec4& destRectPixels) {
		widget->setPosition(CEGUI::UVector2(CEGUI::UDim(destRectPerc.x, destRectPixels.x), CEGUI::UDim(destRectPerc.y, destRectPixels.y)));
		widget->setSize(CEGUI::USize(CEGUI::UDim(destRectPerc.z, destRectPixels.z), CEGUI::UDim(destRectPerc.w, destRectPixels.w)));
	}
}